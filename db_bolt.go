...
func NewBoltDB(config *Config) (*BoltDB, error) {
	...
	if err := os.MkdirAll(config.LibPath, 0600); err != nil {
		if !os.IsExist(err) {
			return nil, fmt.Errorf("BoltDB: Failed to create repback lib directory: %w", err)
		}
	}

	db, err := bolt.Open(filepath.Join(config.LibPath, "repbak.db"), 0600, &bolt.Options{Timeout: 2 * time.Second})
	if err != nil {
		return nil, fmt.Errorf("BoltDB: Failed to open bolt database %s: %w", filepath.Join(config.LibPath, "repbak.db"), err)
	}
